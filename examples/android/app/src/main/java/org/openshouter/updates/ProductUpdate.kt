...
object ProductUpdate {
    ...
    const val RELEASES_API =
        "https://api.github.com/repos/edwardlthompson/OpenShouter/releases/latest"
    const val RELEASES_PAGE =
        "https://github.com/edwardlthompson/OpenShouter/releases/latest"
    ...
    fun parseApkVersion(name: String): String? {
        val match = Regex("openshouter-(\\d+\\.\\d+\\.\\d+)-foss\\.apk", RegexOption.IGNORE_CASE)
            .find(name.trim())
    ...
    fun installUrl(assetUrl: String, htmlUrl: String?): String {
        if (assetUrl.isNotBlank()) return assetUrl
        if (!htmlUrl.isNullOrBlank()) return htmlUrl
