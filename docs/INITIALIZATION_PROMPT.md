...
**Purpose & Goals:** FOSS Android TTS announcer that reads notifications, looping caller ID, battery/power events, geofences, mute gestures, and quiet hours — Voice Notify plus classic Shouter, without Google Play Services.

**Stakeholders:** People who cannot look at the screen while driving, cooking, or using accessibility TTS; maintainers shipping F-Droid builds. **Non-goals:** Play Store exclusivity, cloud sync, SMS reading, Google Play Services. **Success metrics:** Shouter hardware/telephony parity (BUILD_PLAN Sprints 2–8), Voice Notify-class notification filters, zero GMS deps.
